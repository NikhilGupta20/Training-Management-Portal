...
// Create registration table - MODIFIED: residential_phone made NULLable
$sql = "CREATE TABLE IF NOT EXISTS registration (
    ...
    residential_phone VARCHAR(20), -- MODIFIED: Removed NOT NULL constraint
    delhi_address TEXT,
...
$sql = "CREATE TABLE IF NOT EXISTS participant_users (
    id INT(6) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    userid VARCHAR(50) NOT NULL,
...
if (isset($_GET['generate_otp']) && isset($_GET['email'])) {
    ...
    try {
        ...
        // Create a message
        $message = (new Swift_Message('Your NCRB Registration OTP'))
...
// SMS/WhatsApp OTP Generation - FIXED: Improved error handling
...
if (isset($_GET['ajax'])) {
    ...
    if ($_GET['ajax'] == 'get_rank' && isset($_GET['category'])) {
        ...
        $query = "SELECT rank, rank_hindi FROM ranks WHERE category_name = '$category' ORDER BY hierarchy_level";
    ...
    if ($_GET['ajax'] == 'get_events' && isset($_GET['course'])) {
        ...
        if ($result && $result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $options .= '<option value="' . $row['id'] . '" data-start="' . $start . '" data-end="' . $end . '">' . 
                           $start . ' to ' . $end . '</option>';
            }
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['verify_otp']) && !isset($_POST['verify_sms_otp'])) {
    ...
    if (empty($_POST['captcha'])) {
        ...
        if (strcasecmp($userCaptcha, $sessionCaptcha) !== 0) {
            ...
            if ($formData['category_name'] === 'Other' || $formData['category_name_hindi'] === 'अन्य') {
                $formData['category_name'] = $formData['other_category'];
                $formData['category_name_hindi'] = $formData['other_category_hindi'];
            }
            
            // Handle "Other" rank
            if ($formData['rank'] === 'Other' || $formData['rank_hindi'] === 'अन्य') {
                $formData['rank'] = $formData['other_rank'];
                $formData['rank_hindi'] = $formData['other_rank_hindi'];
            }
            ...
            $course_name_hindi = $formData['course_name_hindi'];
            $course_code = '';
            
            $course_result = $conn->query("SELECT course_code FROM training_events WHERE course_name = '$course_name' LIMIT 1");
            ...
            // Generate registration ID and userid (which will be the same)
            if (empty($error)) {
                // Determine year and month from start_date
                if ($startDateMySQL) {
                    $dateObj = DateTime::createFromFormat('Y-m-d', $startDateMySQL);
                    $year = $dateObj->format('Y');
                    $month = $dateObj->format('m');
                } else {
                    // This should not happen because start_date is required and validated
                    $year = date('Y');
                    $month = date('m');
                }

                // Get the next sequential number for this course in this year and month
                $likePattern = "NCRB-{$year}-{$month}-{$course_code}-%";
                $max_id = 0;
                $stmt = $conn->prepare("SELECT MAX(CAST(SUBSTRING_INDEX(registration_id, '-', -1) AS UNSIGNED)) as max_id FROM registration WHERE registration_id LIKE ?");
                if ($stmt) {
                ...
                if (empty($error)) {
                    $next_id = $max_id + 1;
                    if ($next_id > 9999) {
                        $error = "Maximum registrations for this course this month exceeded. Please contact support.";
                    } else {
                        $unique_id = str_pad($next_id, 4, '0', STR_PAD_LEFT);
                        $registration_id = "NCRB-{$year}-{$month}-{$course_code}-{$unique_id}";
                        $formData['registration_id'] = $registration_id;
                        $formData['userid'] = $registration_id;   // Set userid same as registration_id
                        ...
                        if ($valid) {
                            ...
                            try {
                                ...
                                // FIXED: Handle phone as optional
                                $residential_phone_value = !empty($formData['residential_phone']) ? $formData['residential_phone'] : NULL;
                                
                                // FIXED: Corrected column count (25 columns)
                                ...
                                // FIXED: Corrected number of parameters (25 bindings)
                                $stmt->bind_param(
                                    "sssssssssssssssssssssssss", // 25 characters
                                    $formData['userid'], 
                                    ...
                                    $residential_phone_value, // MODIFIED: Using optional phone value
                                    $formData['delhi_address'], 
                                ...
                                if ($stmt->execute()) {
                                    ...
                                    $user_sql = "INSERT INTO participant_users (userid, password) VALUES (?, ?)";
                                    ...
                                    if ($user_stmt->execute()) {
                                        try {
                                                ...
                                                <div class="email-container">
                                                    ...
                                                    <div class="email-body">
                                                        ...
                                                        <div class="credentials">
                                                            <p><strong>Registration ID / User ID:</strong> ' . $registration_id . '</p>
                                            ...
                                            $message = (new Swift_Message('Your Registration Details for NCRB Course'))
                                                ->setFrom(['noreply@ncrb.gov.in' => 'NCRB Registration'])
                                            ...
                                            if ($numSent) {
                                                // Build SMS/WhatsApp message body
                                                ...
                                                $smsBody .= "User ID: " . $registration_id . "\n";
                                                $smsBody .= "Password: " . $password . "\n";
                                                
                                                $smsSuccess = false;
                                                $waSuccess = false;
                                                
                                                // Send via SMS only if phone exists
                                                if (!empty($formData['residential_phone'])) {
                                                    try {
                                                        $twilio->messages->create(
                                                            "+91" . $formData['residential_phone'],
                                                            [
                                                                'from' => '+19472148608',
                                                                'body' => $smsBody
                                                            ]
                                                    ...
                                                    try {
                                                        $twilio->messages->create(
                                                            "whatsapp:+91" . $formData['residential_phone'],
                                                            [
                                                                'from' => 'whatsapp:+14155238886',
                                                                'body' => $smsBody
                                                            ]
                                                ...
                                                $conn->rollback();
                                                $error = "Registration saved but email could not be sent to {$formData['email']}. Please contact support with your registration ID: {$registration_id}";
                                            }
                                        } catch (Exception $e) {
                                            $conn->rollback();
                                            $error = "Registration saved but email could not be sent. Please contact support with your registration ID: {$registration_id}";
                                        }
                                    } else {
                                        $conn->rollback();
                                    ...
                                    $conn->rollback();
                                    $error = "Error saving registration data. Please try again. Error: " . $stmt->error;
                                }
                            } catch (Exception $e) {
                                $conn->rollback();
...
<body>
            ...
            <div class="form-body">
                ...
                <form method="post" id="registrationForm">
                    ...
                    <div class="form-section">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6 mb-3">
                                <label for="training_event" class="form-label required">Select Batch</label>
                                <select class="form-select" id="training_event" name="training_event" required>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-12 mb-3">
                                ...
                                <textarea class="form-control" id="course_expectation" name="course_expectation" rows="2" required><?php echo htmlspecialchars($formData['course_expectation']); ?></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-6 mb-3">
                                <!-- MODIFIED: Removed "required" from phone field -->
    ...
    $(document).ready(function() {
        ...
        // Load ranks for category
        function loadRanksForCategory(category, isHindi) {
            $('#rank, #rank_hindi').prop('disabled', true);
            
            $.ajax({
                url: 'form-registration.php?ajax=get_rank&category=' + encodeURIComponent(category),
                type: 'GET',
                dataType: 'json',
        ...
        $('#course_name, #course_name_hindi').change(function() {
            ...
            const courseName = isHindi ? $('#course_name').val() : $(this).val();
            if (courseName) {
                $('#training_event').prop('disabled', true).html('<option value="">Loading batches...</option>');
                
                $.ajax({
                    url: '?ajax=get_events&course=' + encodeURIComponent(courseName),
                    type: 'GET',
                    success: function(data) {
                        $('#training_event').html(data).prop('disabled', false);
                        $('#start_date, #end_date').val('');
                    },
                    error: function() {
                        $('#training_event').html('<option value="">Error loading batches</option>');
                    }
                });
            } else {
                $('#training_event').html('<option value="">Select course first</option>');
                $('#start_date, #end_date').val('');
            }
        });
        
        // Batch selection handler
        $('#training_event').change(function() {
        ...
        $('#send-otp-btn').click(function() {
            ...
            $.ajax({
                url: 'form-registration.php?generate_otp=1&email=' + encodeURIComponent(email),
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    if (data.status === 'success') {
        ...
        $('#send-sms-otp-btn').click(function() {
          ...
          if (!phone) {
            $('#sms-otp-message').html('<div class="text-danger">Please enter phone number</div>');
            return;
          }
          
          if (!/^\d{10}$/.test(phone)) {
            $('#sms-otp-message').html('<div class="text-danger">Invalid phone number format (10 digits required)</div>');
            return;
          }
          ...
          $.ajax({
            url: 'form-registration.php?generate_sms_otp=1&phone=' + phone,
            type: 'GET',
            dataType: 'json',
            success: function(data) {
              if (data.status === 'success') {
        ...
        $('#verify-sms-otp-btn').click(function() {
          ...
          if (!/^\d{6}$/.test(otp)) {
            $('#sms-otp-message').html('<div class="text-danger">Invalid OTP format (6 digits required)</div>');
            return;
          }
        ...
        $('#registrationForm').submit(function(e) {
            ...
            // Validate phone number if provided - MODIFIED: Not required
            const phone = $('#residential_phone').val();
            ...
            if (($('#category_name').val() === 'Other' || $('#category_name_hindi').val() === 'अन्य') && 
                (!$('#other_category').val() || !$('#other_category_hindi').val())) {
                $('.other-category-container input').addClass('is-invalid');
                valid = false;
            }
        ...
        $('#refresh-captcha').click(function() {
            $.get('form-registration.php?refresh_captcha=1', function(data) {
