...
$course_name = htmlspecialchars($courseName ?: ($participant['course_name'] ?? 'Not specified'));
...
if ($course_name) {
    try {
        // Get course code from training_events
        $stmt = $pdo->prepare("SELECT DISTINCT course_code FROM training_events WHERE course_name = ?");
        $stmt->execute([$course_name]);
        $courseCodes = $stmt->fetchAll(PDO::FETCH_COLUMN);
        
        if (!empty($courseCodes)) {
            $placeholders = rtrim(str_repeat('?,', count($courseCodes)), ',');
...
<head>
    ...
    <title>Course Materials - NCRB Training Portal</title>
    <!-- Favicon links -->
    ...
    <div class="fixed-header">
        ...
        <div class="nav-container">
            <div class="nav-content">
                <div class="nav-title">National Crime Records Bureau - Study Materials</div>
    ...
    <div class="dashboard">
        <div class="dashboard-container">
            <div class="dashboard-header">
                ...
                <p>Study materials for your training program</p>
            </div>
            ...
            <?php if ($hasMaterials): ?>
                <div class="materials-grid">
                    <?php foreach ($materials as $material): 
                        ...
                        if (in_array($fileExt, ['pdf', 'doc', 'docx', 'txt', 'rtf', 'odt', 'xls', 'xlsx', 'csv', 'ppt', 'pptx', 'odp', 'ods', 'md'])) {
                            $icon = 'fa-file-alt';
                        } elseif (in_array($fileExt, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'tiff', 'tif', 'webp', 'svg', 'ico'])) {
                            $icon = 'fa-file-image';
                        } elseif (in_array($fileExt, ['zip', 'rar', '7z', 'tar', 'gz', 'bz2', 'xz', 'tgz', 'dmg', 'iso'])) {
                            $icon = 'fa-file-archive';
                        } elseif (in_array($fileExt, ['mp3', 'wav', 'aac', 'm4a', 'ogg', 'flac', 'amr'])) {
                            $icon = 'fa-file-audio';
                        } elseif (in_array($fileExt, ['mp4', 'mkv', 'mov', 'avi', 'webm', 'm4v', '3gp', 'flv'])) {
                            $icon = 'fa-file-video';
                        } elseif (in_array($fileExt, ['html', 'htm', 'css', 'js', 'php', 'py', 'java', 'c', 'cpp', 'h', 'cs'])) {
                            $icon = 'fa-file-code';
                        }
                    ?>
