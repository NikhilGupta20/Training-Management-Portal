...
// Create training_events table with new columns
$sql = "CREATE TABLE IF NOT EXISTS training_events (
    ...
    status ENUM('Completed', 'Active', 'Incoming') DEFAULT 'Incoming',
...
function getNextFinancialYear($date) {
    $year = date('Y', strtotime($date));
    $month = date('m', strtotime($date));
    return ($month < 4) ? $year . '-' . ($year + 1) : ($year + 1) . '-' . ($year + 2);
}

$financialYear = getNextFinancialYear(date('Y-m-d'));
[$startYear, $endYear] = array_map('intval', explode('-', $financialYear));

// Fetch training events ordered by status and date
$events = [];
$sql = "SELECT * FROM training_events 
        ...
        ORDER BY 
            FIELD(status, 'Active', 'Incoming', 'Completed'),
            start_date ASC";
...
<head>
    ...
    <title>Next Year Training Calendar | NCRB</title>
    ...
    <style>
        ...
        * {
        ...
        /* Modal Styles - Same as home-page.php */
        .modal {
            display: none;
            position: fixed;
            top: 0;
        ...
        /* Developer Contact - Same as home-page.php */
        .developer-contact {
            text-align: center;
            margin-bottom: 20px;
        }
...
<body>
    ...
    <div class="dashboard">
        <div class="dashboard-header">
            <h1 class="page-title">Training Calendar for Next Academic Year</h1>
        </div>
        ...
        <div class="pdf-viewer-container">
            ...
            <div class="pdf-viewer">
                <div class="pdf-placeholder">
                    ...
                    <h3>PDF Not Available Yet</h3>
                    <p>The training calendar PDF will be available once uploaded</p>
                </div>
            </div>
            <p class="pdf-note">This PDF will contain the complete training schedule for the next academic year</p>
        </div>
